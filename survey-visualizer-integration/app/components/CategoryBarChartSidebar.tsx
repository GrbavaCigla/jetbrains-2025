...
export default function CategoryBarChartSidebar({
  defaultGroupChecked = false,
  onChange,
  categories,
}: Props) {
  ...
  const toggles = categories.map((val) => (
    <label className="label font-semibold px-4 pb-4" key={val}>
      ...
      <input
        ...
        className="checkbox"
        defaultChecked={true}
        onChange={(e) => handleCategoriesToggleChange(e, val)}
      />
    </label>
  ));

  return (
    <div className="bg-white border-(length:--border) flex flex-col md:w-80 overflow-hidden h-full">
      ...
      <label className="label font-semibold px-4 pb-4 border-b-(length:--border) border-black">
        ...
        <input
          ...
          onChange={handleGroupToggleChange}
          className="toggle"
        />
      </label>
